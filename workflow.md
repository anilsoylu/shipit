...
- Model routing (asset vs throughput): spend the strongest model on durable artifacts — specs, plans, skills, swap-contract decisions, anything hard to reverse — because you pay once and reuse the output. Route disposable work (scaffold grunt, bulk subagent tasks, drafts, summaries) to cheaper models. Capability lives in procedures + verification, not in model loyalty: this pack's contracts and checklists are written to run on any model.
