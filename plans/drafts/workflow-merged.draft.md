<!--
DRAFT — workflow-generated starting point for plans/004 (merged workflow.md).
NOT final. Everything from the top through "## 7. Done Definition" is grounded
(the Claude Code Orchestration Primer's features were confirmed against the
changelog feature flags + the dynamic-workflows announcement) — keep it.

REQUIRED CHANGE before this becomes docs/workflow.md:
  Replace the entire "## Stack Options (verified)" section at the bottom with a
  SHORT pointer to `stack-picker.md` and the LLM Docs Registry in `skills.md`.
  Do NOT keep a second options table here — its skill slugs (hairyf/skills,
  mcollina/skills, yusukebe/hono-skill, deckardger/..., jezweb/...,
  getsentry/sentry-for-ai, posthog/posthog-for-claude, openrouterteam/...,
  get-convex/..., revenuecat/ai-toolkit, firebase/agent-skills) were NOT verified
  and several conflict with the ground truth in plans/research-findings.md.
  One source of truth for options = skills.md registry; workflow.md just links it.
Keep the two Claude Code source links in Official References.
-->
...
- Create Fastify TypeScript service (default; Hono is a valid swap for edge/serverless).
- Add health route.
- Add auth middleware (default Clerk).
