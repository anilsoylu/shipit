<!--
DRAFT — workflow-generated starting point for plans/005 (stack-picker.md).
The skill-slug anchor table at the bottom has ALREADY been corrected against the
API-resolved Authoritative slugs table in plans/research-findings.md. Use those
slugs as-is. If you re-verify, note the confirmed real slugs:
  - Stripe = `stripe/ai`; RevenueCat = `revenuecat/ai-toolkit`;
    Sentry SDK setup = `getsentry/sentry-for-ai`; OpenRouter = `openrouterteam/agent-skills`;
    Vercel AI SDK = `vercel/ai`; Resend = `resend/resend-skills`;
    Drizzle = `bobmatnyc/claude-mpm-skills` (community — or keep llms.txt only).
Keep the structure and prose.
-->
...
**LLM Docs Registry** in `skills.md`. Quick anchors (slugs API-resolved from
`plans/research-findings.md`):
...
| Drizzle | `bobmatnyc/claude-mpm-skills` (community) | https://orm.drizzle.team/llms.txt |
...
| Sentry | `getsentry/sentry-for-ai` (SDK setup) | https://docs.sentry.io/llms.txt |
